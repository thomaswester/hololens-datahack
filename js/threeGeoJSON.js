...
//hack to focus on portland,or map, instead of world space
var MIN_LON = -122.4;
var MAX_LON = -122.9;
...
function drawThreeGeo(json, radius, shape, options) {
    ...
    for (var geom_num = 0; geom_num < json_geom.length; geom_num++) {
        ...
        if (json_geom[geom_num].type == 'Polygon') {                        
            ...
            console.log("what about: ", json_geom[geom_num].type);
...
function drawLine(x_values, y_values, z_values, options) {
    ...
    //nShape.moveTo(( x_values[0], y_values[0] ));
    
    var pts = [];
    for (var i = 1; i < x_values.length; i++) {
        pts.push( new THREE.Vector2 ( x_values[i], y_values[i] )  );
        
        //nShape.lineTo ( x_values[i], y_values[i] );
    }

    var nShape = new THREE.Shape( pts );
    var rectGeom;
    ...
    for (var c = 0; c < rectGeom.faces.length; c++) {
        var face = rectGeom.faces[c];
        face.color.set( options.color );
    }

    var rectMesh = new THREE.Mesh( rectGeom );
    meshes.push( rectMesh);

    //core.scene.add( rectMesh );
...
/*
    ...
    var nMesh = new THREE.Mesh(line_geom, line_material);
    core.scene.add(nMesh);
*/
    clearArrays();
}
